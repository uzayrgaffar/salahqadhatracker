...
const Progress = () => {
  ...
  const getPrayerCounts = (): any[] => {
    return dates.map((date) => {
      const counts = dailyPrayerCounts[date] || { fajr: 0, dhuhr: 0, asr: 0, maghrib: 0, isha: 0, witr: 0 }
      return Object.values(counts).reduce((total, count) => (total as number) + (count as number), 0)
    })
  }
  ...
  const totalQadhaPrayed: any = prayerCounts.reduce((sum:any, count:any) => sum + count, 0)
